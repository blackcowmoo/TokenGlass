...
impl CodexAppServer {
    ...
    fn request(&self, method: &str, params: Value) -> Result<Value, String> {
        ...
        let message = receiver.recv_timeout(Duration::from_secs(20)).map_err(|_| "Codex 응답 시간이 초과되었습니다.".to_string())?;
        if let Some(error) = message.get("error") {
            return Err(error.get("message").and_then(Value::as_str).unwrap_or("Codex 요청에 실패했습니다.").to_string());
        }
        message.get("result").cloned().ok_or("Codex 응답에 결과가 없습니다.".to_string())
    }
}
...
fn start_chatgpt_login(app_server: tauri::State<'_, CodexAppServer>) -> Result<ChatGptLogin, String> {
    ...
    Ok(ChatGptLogin {
        login_id: result.get("loginId").and_then(Value::as_str).ok_or("로그인 ID를 받지 못했습니다.")?.to_string(),
        auth_url: result.get("authUrl").and_then(Value::as_str).ok_or("로그인 URL을 받지 못했습니다.")?.to_string(),
    })
}
...
fn fetch_chatgpt_subscription_usage(app_server: tauri::State<'_, CodexAppServer>) -> Result<ChatGptSubscriptionUsage, String> {
    ...
    if let Some(items) = rate_limits.get("rateLimitsByLimitId").and_then(Value::as_object) {
        for (limit_id, item) in items {
            let primary = item.get("primary");
            limits.push(SubscriptionLimit {
                limit_id: limit_id.clone(),
                label: item.get("limitName").and_then(Value::as_str).map(str::to_owned),
                used_percent: primary.and_then(|value| value.get("usedPercent")).and_then(Value::as_f64),
                window_duration_mins: primary.and_then(|value| value.get("windowDurationMins")).and_then(Value::as_u64),
                resets_at: primary.and_then(|value| value.get("resetsAt")).and_then(Value::as_i64),
            });
        }
    }
    if limits.is_empty() {
        if let Some(item) = rate_limits.get("rateLimits") {
            let primary = item.get("primary");
            limits.push(SubscriptionLimit {
                limit_id: item.get("limitId").and_then(Value::as_str).unwrap_or("codex").to_string(),
                label: item.get("limitName").and_then(Value::as_str).map(str::to_owned),
                used_percent: primary.and_then(|value| value.get("usedPercent")).and_then(Value::as_f64),
                window_duration_mins: primary.and_then(|value| value.get("windowDurationMins")).and_then(Value::as_u64),
                resets_at: primary.and_then(|value| value.get("resetsAt")).and_then(Value::as_i64),
    ...
    let daily_usage = usage.get("dailyUsageBuckets").and_then(Value::as_array).into_iter().flatten().map(|bucket| DailyTokenUsage {
        start_date: bucket.get("startDate").and_then(Value::as_str).unwrap_or_default().to_string(),
        tokens: bucket.get("tokens").and_then(Value::as_u64).unwrap_or(0),
    }).collect();

    Ok(ChatGptSubscriptionUsage {
        email: account_info.get("email").and_then(Value::as_str).map(str::to_owned),
        plan_type: account_info.get("planType").and_then(Value::as_str).map(str::to_owned),
        limits,
        lifetime_tokens: usage.pointer("/summary/lifetimeTokens").and_then(Value::as_u64),
        peak_daily_tokens: usage.pointer("/summary/peakDailyTokens").and_then(Value::as_u64),
        daily_usage,
    })
}
...
async fn fetch_openai_usage(admin_key: String) -> Result<OpenAiUsage, String> {
    ...
    let period_start = time::Date::from_calendar_date(utc_now.year(), utc_now.month(), 1)
        ...
        .assume_utc()
        .unix_timestamp();
    let today_start = time::Date::from_calendar_date(utc_now.year(), utc_now.month(), utc_now.day())
    ...
    let usage: Value = usage_response.json().await.map_err(|error| format!("사용량 응답을 읽을 수 없습니다: {error}"))?;
    ...
    let costs: Value = costs_response.json().await.map_err(|error| format!("비용 응답을 읽을 수 없습니다: {error}"))?;

    let mut input_tokens = 0_u64;
    let mut output_tokens = 0_u64;
    let mut model_tokens = std::collections::BTreeMap::<String, u64>::new();
    for bucket in usage.get("data").and_then(Value::as_array).into_iter().flatten() {
        for result in bucket.get("results").and_then(Value::as_array).into_iter().flatten() {
            let input = result.get("input_tokens").and_then(Value::as_u64).unwrap_or(0);
            let output = result.get("output_tokens").and_then(Value::as_u64).unwrap_or(0);
            input_tokens += input;
            output_tokens += output;
            let model = result.get("model").and_then(Value::as_str).unwrap_or("Other");
    ...
    for bucket in costs.get("data").and_then(Value::as_array).into_iter().flatten() {
        let is_today = bucket.get("start_time").and_then(Value::as_i64).unwrap_or(0) >= today_start;
        for result in bucket.get("results").and_then(Value::as_array).into_iter().flatten() {
            let amount = result.pointer("/amount/value").and_then(Value::as_f64).unwrap_or(0.0);
            total_billed += amount;
            if is_today {
                today_usage += amount;
            }
